...
namespace OpenDental{
	...
	public class FormApptEdit : ODForm {
		...
		private void FormApptEdit_Load(object sender, System.EventArgs e) {
			...
			strBTime=new StringBuilder(Appointments.ConvertPatternFrom5(AptCur.Pattern));
		...
		private void butDeleteProc_Click(object sender,EventArgs e) {
			...
			if(isProcDeleted) {
				...
				//the same procs, the Appointments.Update(old,new) will not update those fields due to them being identical.
		...
		private bool AptTypeHelper() {
			...
			if(aptTypeCur.Pattern!=null && aptTypeCur.Pattern!="") {
				strBTime=new StringBuilder(Appointments.ConvertPatternFrom5(aptTypeCur.Pattern));
			}
