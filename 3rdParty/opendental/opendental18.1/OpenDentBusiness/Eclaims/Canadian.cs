...
//Warning: this path is not handled properly if trailing slash is missing:
...
#endif
			string errorMsg="";
			string result=PassToIca(strb.ToString(),clearinghouseClin,isAutomatic,out errorMsg);
		...
		///<summary>Used for ITRANS and Claimstream.  Takes a string, creates a file, and drops it into the clearinghouse export path.
		///Waits for the response, and then returns it as a string.
		///The errorMsg will provide translated error details if a response was received and an error occurred,
		///otherwise errorMsg will be blank to indicate success.</summary>
		public static string PassToIca(string msgText,Clearinghouse clearinghouseClin,bool isAutomatic,out string errorMsg) {
			errorMsg="";
			...
			if(isClaimstream) {
				...
				if(!File.Exists(certFilePath)) {
					try {
						...
						errorMsg=Lans.g("Canadian","Failed to export OPENDENTAL.pem to Claimstream export path.")+"  "+ex.Message;
						return "";//Return empty response, since we never received one.
					}
				}
			}
